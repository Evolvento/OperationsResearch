...
def plan_optimization():
    ...
    # for k in range(len(evaluation_matrix)):
    #     print(evaluation_matrix[k])
    while min_elem_matrix(evaluation_matrix)[0] < 0:
        ...
        print(tabulate.tabulate(reference_plan))
...
transportation_costs = [list(map(int, input("Введите затраты на перевозку: ").split())) for i in range(len(
    number_units_suppliers))]
...
# for k in range(len(reference_plan)):
#     print(*reference_plan[k])
# for g in range(len(reference_plan)):
#     print(*reference_plan[g])
# print("-----------------------")
plan_optimization()
print("Минимальные затраты на перевозку:", goal_function_calculation())
